...
public class T3 {
    public static int maximumsSplicedArray(int[] nums1, int[] nums2) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            cnt1 += nums1[i];
            cnt2 += nums2[i];
            f2[i] = nums2[i] - nums1[i];
        }

        int dp1 = getInts(n, f2);
    ...
    private static int getInts(int n, int[] f) {
        int[] dp1 = new int[n + 1];
        int res = -1;
        for (int i = 1; i <= n; i++) {
            if (f[i - 1] > f[i - 1] + dp1[i - 1]) {
                dp1[i] = f[i - 1];
            } else
                dp1[i] = dp1[i - 1] + f[i - 1];
